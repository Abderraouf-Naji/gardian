...
def detect_feature_dimensions(file_path):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            if line.strip():
                ...
                print(f"  Total features: {sparse_len + dense_len}")
                
                dimensions[f"sample_{i+1}"] = {
                    "sparse": sparse_len,
                    "dense": dense_len,
                    "total": sparse_len + dense_len
                }
    ...
    if dimensions:
        ...
        print("  query_feat_dim: 384  # all-MiniLM-L6-v2 default")
        print(
            f"  question_types: {list(ORDERED_QUESTION_TYPES)} "
            f"(N={N_QTYPES}) — evaluation.question_types must match this order exactly."
        )
