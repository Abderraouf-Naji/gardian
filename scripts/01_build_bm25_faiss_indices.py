...
# Eval-only BEIR corpora: buildable with --dataset <name>, never merged into
# the unified 3M index (keeps existing training/QA indices unchanged).
OPTIONAL_CORPORA: Dict[str, str] = {
    "trec_covid": "data/corpus_trec_covid.jsonl",
    # BEIR retrieval collections, both small enough to index beside the paper
...
ALL_CORPORA: Dict[str, str] = {**CORPORA, **OPTIONAL_CORPORA}
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"{', '.join(ALL_CORPORA.keys())}, ``{UNIFIED_NAME}``, or ``all`` "
            "(default: paper corpora only; optional BEIR sets like trec_covid "
    ...
    organize_existing_index_artifacts(
        indices_dir, list(ALL_CORPORA.keys()) + [UNIFIED_NAME]
    )
    ...
    valid_per_corpus = set(ALL_CORPORA.keys())

    # ``all`` builds paper corpora only (never optional BEIR sets / never
    # pulls them into the unified merge).
    if dataset_arg == "all":
        source_corpora = CORPORA
    elif dataset_arg in OPTIONAL_CORPORA:
        source_corpora = {dataset_arg: OPTIONAL_CORPORA[dataset_arg]}
    elif dataset_arg in CORPORA or dataset_arg == UNIFIED_NAME:
        source_corpora = CORPORA
    else:
        raise ValueError(
    ...
    corpora_paths: Dict[str, pathlib.Path] = {
        name: pathlib.Path(path_str) for name, path_str in source_corpora.items()
    }
    ...
    if not available_corpora and dataset_arg != UNIFIED_NAME:
        raise FileNotFoundError(
            "No corpus files found. Run Script 00 (or 00b for trec_covid) first."
        )
    ...
    if dataset_arg != "all":
        if dataset_arg == UNIFIED_NAME:
            ...
            if dataset_arg in OPTIONAL_CORPORA:
                path = pathlib.Path(OPTIONAL_CORPORA[dataset_arg])
                if not path.exists():
                    raise FileNotFoundError(
                        f"--dataset {dataset_arg!r} requested but corpus file "
                        f"not found: {path}. Run scripts/00b_download_trec_covid.py"
                    )
                available_corpora = {dataset_arg: path}
            elif dataset_arg in available_corpora:
